...
import { PIVOT_ALGORITHM_VERSION, PIVOT_METHOD } from "@/lib/analysis/pivots";
...
const PHASES: [string, string, "live" | "next" | "planned"][] = [
  ...
  ["5", "Candidate count generation", "next"],
  ["6", "Fibonacci engine and confluence zones", "planned"],
  ...
  return (
    <section className="card">
      <div className="card-h">
        ...
        <span className={`chip ${run?.error ? "neg" : "pos"}`}>{run?.error ? "Worker error" : "Pivot engine live"}</span>
        <span className="card-s ml-auto">Worker runs every 5 minutes · {PIVOT_ALGORITHM_VERSION}</span>
      </div>
      <div className="grid grid-cols-2 gap-px border-b border-line bg-line lg:grid-cols-4">
        {tfs.map(([k, label]) => {
          ...
          return (
            <div key={k} className="bg-panel px-5 py-4">
              <div className="text-[13px] text-fg-2">{label} pivots cached</div>
      ...
      <p className="px-5 py-4 text-[12.5px] leading-relaxed text-fg-3">
        ...
        While the backfill runs, most rows are refreshed every few minutes. 1H, 4H and 1M pivots are computed when a chart is opened.
        {run?.error && <span className="text-neg"> Last error: {run.error}</span>}
      </p>
      <SourceFooter source="Viridia pivot engine on stored Massive bars" updated={fmtDateTime(run?.finished_at)} method={PIVOT_METHOD} />
    </section>
  );
}
